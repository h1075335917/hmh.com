...
- [Java9开始G1就成了默认的垃圾回收器](#g1成为默认垃圾回收器)，G1 是以一种低延时的垃圾回收器来设计的，旨在避免进行 Full GC,但是 Java9 的 G1 的 FullGC 依然是使用单线程去完成标记清除算法,这可能会导致垃圾回收期在无法回收内存的时候触发 Full GC。
...
- 释放未使用内存功能
- 在 Java 11 中实验性引入的ZGC 在实际的使用中存在未能主动将未使用的内存释放给操作系统的问题。
...
- Java12 为 switch 表达式引入了类似 lambda 语法条件匹配成功后的执行块，不需要多写 break
- Java13 提供了 `yield` 来在 block 中返回值
...
- JDK 13，instanceof 模式匹配（预览）
- JDK 14，instanceof 增强（二次预览）
- JDK 15，instanceof 模式匹配（三次预览）
- JDK16，正式，模式变量不再隐式为 final，你可以对 `instanceof` 中的变量值进行修改。

##### 记录类型（正式）

- JDK 14，record 关键字（预览）。
...
- 在 Java 18 之后，可以通过 `@snippet` 标签来做这件事情。`@snippet` 这种方式生成的效果更好且使用起来更方便一些。
...
- 四种创建虚拟线程的方法：通过列举的 4 种创建虚拟线程的方式可以看出，官方为了降低虚拟线程的门槛，尽力复用原有的 `Thread` 线程类，这样可以平滑的过渡到虚拟线程的使用。
...
- [Dragonwell OpenJDK](https://www.aliyun.com/product/dragonwell)
...
- 有商业支持需求的选择Oracle JDK。 
- 如果对性能方面有更高要求的选Azul Platform Prime, 即zing jvm。
- 免费用户无特殊要求的选Temurin OpenJDK即可。
- 有特殊架构需求的，如Mac m1（aarch64）有时释出版本受限，可选择zulu OpenJDK。
